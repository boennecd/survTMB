...
struct test_gsm_fam_util {
  std::vector<double> g_log, gp, gp_g, gpp_gp, gpp, d_gp_g, d_gpp_gp;
  ...
  test_gsm_fam_util(std::vector<double> const &eta){
    ...
    for(auto &e : eta){
      ...
      d_gp_g  .emplace_back(f.d_gp_g());
      d_gpp_gp.emplace_back(f.d_gpp_gp());
    }
  }
};
} // namespace
...
context("testing gsm") {
  test_that("ph link is correct") {
    /*
     ...
     dput(gp / g(eta))
     dput(gpp / gp)
     dput(sapply(eta, function(x) hessian(g_log, x, method.args=list(eps = 1e-8))))
     gppp <- sapply(eta, function(x) pracma::fderiv(g, x, n = 3))
     dput((gp * gppp - gpp^2) / gp^2)
     dput(-exp(eta))
     */
    std::vector<double> const
      ...
      d_gp_g   = { -0.0497870683678644, -0.135335283236549, -0.367879441170939,
                   -1, -2.71828182846179, -7.38905609892995, -20.0855369231878 },
      d_gpp_gp = { -0.0497870683678639, -0.135335283236613, -0.367879441171442,
                   -1, -2.71828182845905, -7.38905609893065, -20.0855369231877 };
    ...
    for(size_t i = 0; i < eta.size(); ++i){
      ...
      expect_equal_eps(d_gp_g  [i], fam.d_gp_g[i], 1e-5);
      expect_equal_eps(d_gpp_gp[i], fam.d_gpp_gp[i], 1e-5);
    }
  }
  ...
  test_that("po link is correct") {
    /*
     ...
     dput(gpp / gp)
     dput(sapply(eta, function(x) hessian(g_log, x, method.args=list(eps = 1e-8))))
     dput(- exp(eta) / (1 + exp(eta))^2)
     gppp <- sapply(eta, function(x) pracma::fderiv(g, x, n = 3))
     dput((gp * gppp - gpp^2) / gp^2)
     dput(- 2 *  exp(eta) / (1 + exp(eta))^2)
     */
    std::vector<double> const
    ...
    gpp_gp   = { 1, 0.90514825363963, 0.761594155948289, 0.462117157236798,
                 ...
                 -0.905148253647757, -1 },
    d_gp_g   = { -4.24835425529159e-18, -0.0451766597309121, -0.104993585403507,
                 -0.196611933241482, -0.25, -0.196611933241482, -0.104993585403507,
                 -0.0451766597309121, -4.24835425529159e-18 },
    d_gpp_gp = { -8.49670851058318e-18, -0.0903533194618243, -0.209987170807013,
                 -0.393223866482964, -0.5, -0.393223866482964, -0.209987170807013,
                 -0.0903533194618243, -8.49670851058318e-18 };
  ...
  test_that("probit link is correct") {
    /*
     ...
     dput(gpp / gp)
     dput(sapply(eta, function(x) hessian(g_log, x, method.args=list(eps = 1e-8))))
     ldiff <- dnorm(-eta, log = TRUE) - pnorm(-eta, log.p = TRUE)
     dput(eta * exp(ldiff) - exp(2 * ldiff))
     gppp <- sapply(eta, function(x) pracma::fderiv(g, x, n = 3))
     dput((gp * gppp - gpp^2) / gp^2)
     */
    std::vector<double> const
    ...
    gpp_gp   = { 40., 3.00000000015909, 2.0000000000769, 0.999999999998486,
                 ...
                 -40 },
    d_gp_g   = { 9.22095252600812e-289, -0.0133332115416628, -0.113548051688565,
                -0.370313714223942, -0.636619772367581, -0.800902334427776, -0.885720899586659,
                -0.929440813213955, -0.999377331621086 },
    d_gpp_gp = { -1, -1, -1, -1, -1, -1, -1, -1, -1 };
