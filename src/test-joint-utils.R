...
# parameters
omega <- if(without_b) numeric() else c(-0.74, 0.76, 0.20)
...
B <- if(without_g)
  numeric() else
    structure(c(0.97, 0.01, -0.07, .02, -0.78, -0.86, 0.98, -0.03),
              .Dim = c(4, 2))
dput(t(B))
...
dput(m_knots <- seq(0, 10, length.out = 3))
m_func <- with(new.env(), {
  ks <- m_knots
...
dput(g_knots <- seq(0, 10, length.out = 4))
g_func <- with(new.env(), {
  ks <- g_knots
