...
get_func_eortc_adfun <- function(link, n_threads, method, param_type)
  make_mgsm_ADFun(
    ...
    sparse_hess = FALSE, param_type = param_type)

get_psqn_obj <-  function(link, n_threads, method, param_type)
  make_mgsm_psqn_obj(
    Surv(y, uncens) ~ trt, cluster = as.factor(center), Z = ~ 1,
...
for(link in c("PH", "PO", "probit"))
  test_that(sprintf("psqn method gives something similar to adfun version with link '%s' and method '%s'", link, m), {
    obj <- get_psqn_obj(link, 2L, method = m, param_type = "DP")
    ...
    if(m == "SNVA"){
      # do the same w/ CP_trans
      obj <- get_psqn_obj(link, 2L, method = m, param_type = "CP_trans")
      expect_s3_class(obj, "mgsm_psqn")
